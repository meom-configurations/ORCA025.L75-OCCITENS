...
!$AGRIF_END_DO_NOT_TREAT
   ...
   !! $Id: fldread.F90 3851 2013-03-27 10:03:54Z smasson $
...
!CDIR COLLAPSE
   ...
   SUBROUTINE wgt_list( sd, kwgt )
      ...
      !!
      INTEGER ::   jw, inestid   ! local integer
      LOGICAL ::   l_found        ! local logical
      !!----------------------------------------------------------------------
      !
...
#endif
      DO jw = 1, nxt_wgt-1
         IF( TRIM(ref_wgts(jw)%wgtname) == TRIM(sd%wgtname) .AND. &
             ref_wgts(jw)%nestid == inestid) THEN
            kwgt = jw
            l_found = .TRUE.
            EXIT
         ENDIF
