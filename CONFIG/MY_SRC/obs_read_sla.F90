MODULE obs_read_sla
   ...
   !! $Id: obs_read_sla.F90 3651 2012-11-26 10:46:39Z cbricaud $
...
CONTAINS
   ...
   SUBROUTINE obs_rea_sla( kformat, &
      ...
      CALL sort_dp_indx( iobstot, &
         &               zdat,     &
         &               iindx   )
      CALL obs_surf_alloc( sladata, iobs, kvars, kextr, kstp, jpi, jpj )
      ...
      IF (lwp) THEN
         ...
         DO jj = 1,8
            IF ( itypmpp(jj) > 0 ) THEN
               WRITE(numout,'(1X,A38,A2,I10)')calttyp(jj),'= ',itypmpp(jj)
            ENDIF
         END DO
         WRITE(numout,'(1X,A50)')'--------------------------------------------------'
