...
!}
         CALL ctl_opn( numstp, cloutput, 'REPLACE', 'FORMATTED', 'SEQUENTIAL', -1, numout, lwp, narea )
      ENDIF
...
!{ JMM
         cloutput='solver.stat'
!        IF(ln_ensemble) cloutput = c_nmem//cloutput
         IF(ln_ensemble) cloutput = TRIM(cloutput)//"."//TRIM(c_nmem)
!}
            CALL ctl_opn( numsol, cloutput, 'REPLACE', 'FORMATTED', 'SEQUENTIAL', -1, numout, lwp, narea )
         ENDIF
         !
