...
#  include "ldfeiv_substitute.h90"
   ...
   !! $Id: traldf_iso_grif.F90 3632 2012-11-22 15:28:42Z acc $
...
CONTAINS
  ...
  SUBROUTINE tra_ldf_iso_grif( kt, kit000, cdtype, pgu, pgv,              &
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) :: zw3d, zftvm       ! 3D workspace
      !!----------------------------------------------------------------------
      !
      IF( nn_timing == 1 )  CALL timing_start('tra_ldf_iso_grif')
      !
      CALL wrk_alloc( jpi, jpj,      z2d ) 
      CALL wrk_alloc( jpi, jpj, jpk, zdit, zdjt, ztfw, zftvm  ) 
      !
...
# endif
         ...
         IF( cdtype == 'TRA' .AND. ln_diaptr ) THEN
           DO jb=1,nptr
              DO jk = 1, jpk
               zftvm(:,:,jk) = zftv(:,:,jk) * btmsk(:,:,jb)
              ENDDO
            IF( jn == jp_tem)   htr_ldf(:,jb) = ptr_sj( zftvm(:,:,:) )        ! 3.3  names
            IF( jn == jp_sal)   str_ldf(:,jb) = ptr_sj( zftvm(:,:,:) )
           ENDDO
         ENDIF
      ...
      CALL wrk_dealloc( jpi, jpj, jpk, zdit, zdjt, ztfw, zftvm  ) 
      !
