...
9000  FORMAT(a,"_",a,"_y",i4.4,"m",i2.2,"d",i2.2,".dimgproc")
      ...
      ! IV.2 Code for XIOS output
      ! ----------------------------------

!-lb      IF( MOD( itmod, nn_trd ) == 0 ) THEN

         IF ( ln_trdmld_instant ) THEN

            ! Temperature values
            CALL iom_put( "ztml"          , tml(:,:)                                      )    ! temperature
            CALL iom_put( "ztmltrd_tot"   , ztmltot(:,:)                                  )    ! total trends
            CALL iom_put( "ztmltrd_res"   , ztmlres(:,:)                                  )    ! residual trends
            CALL iom_put( "ztmltrd_xad"   , tmltrd(:,:,jpmld_xad)                         )    ! zonal. advective trend
            CALL iom_put( "ztmltrd_yad"   , tmltrd(:,:,jpmld_yad)                         )    ! merid. advective trend
            CALL iom_put( "ztmltrd_zad"   , tmltrd(:,:,jpmld_zad)                         )    ! verti. advective trend
            CALL iom_put( "ztmltrd_ldf"   , tmltrd(:,:,jpmld_ldf)                         )    ! horiz. diffusive trend
            CALL iom_put( "ztmltrd_for"   , tmltrd(:,:,jpmld_for)                         )    ! forcing trend
            CALL iom_put( "ztmltrd_zdf"   , tmltrd(:,:,jpmld_zdf)                         )    ! verti. diffusive trend
            CALL iom_put( "ztmltrd_bbc"   , tmltrd(:,:,jpmld_bbc)                         )    ! geothermal flux trend
            CALL iom_put( "ztmltrd_bbl"   , tmltrd(:,:,jpmld_bbl)                         )    ! bottom boundary layer (advective/diffusive)
            CALL iom_put( "ztmltrd_dmp"   , tmltrd(:,:,jpmld_dmp)                         )    ! internal restoring trend
            CALL iom_put( "ztmltrd_npc"   , tmltrd(:,:,jpmld_npc)                         )    ! non penetrative convective adjustment 
            CALL iom_put( "ztmltrd_atf"   , tmltrd(:,:,jpmld_atf)                         )    ! Asselin filter trend
            CALL iom_put( "ztmltrd_xyad"  , tmltrd(:,:,jpmld_xad) + tmltrd(:,:,jpmld_yad) )    ! zon.   + mer.   advective trends
            CALL iom_put( "ztmltrd_lzdf"  , tmltrd(:,:,jpmld_ldf) + tmltrd(:,:,jpmld_zdf) )    ! horiz. + verti. diffusive trends

            ! Salinity values
            !CALL iom_put( "zsml"          , sml(:,:)                                      )    ! salinity
            !CALL iom_put( "zsmltrd_tot"   , zsmltot(:,:)                                  )    ! total trends
            !CALL iom_put( "zsmltrd_res"   , zsmlres(:,:)                                  )    ! residual trends
            !CALL iom_put( "zsmltrd_xad"   , smltrd(:,:,jpmld_xad)                         )    ! zonal. advective trend
            !CALL iom_put( "zsmltrd_yad"   , smltrd(:,:,jpmld_yad)                         )    ! merid. advective trend
            !CALL iom_put( "zsmltrd_zad"   , smltrd(:,:,jpmld_zad)                         )    ! verti. advective trend
            !CALL iom_put( "zsmltrd_ldf"   , smltrd(:,:,jpmld_ldf)                         )    ! horiz. diffusive trend
            !CALL iom_put( "zsmltrd_for"   , smltrd(:,:,jpmld_for)                         )    ! forcing trend
            !CALL iom_put( "zsmltrd_zdf"   , smltrd(:,:,jpmld_zdf)                         )    ! verti. diffusive trend
            !CALL iom_put( "zsmltrd_bbc"   , smltrd(:,:,jpmld_bbc)                         )    ! geothermal flux trend
            !CALL iom_put( "zsmltrd_bbl"   , smltrd(:,:,jpmld_bbl)                         )    ! bottom boundary layer (advective/diffusive)
            !CALL iom_put( "zsmltrd_dmp"   , smltrd(:,:,jpmld_dmp)                         )    ! internal restoring trend
            !CALL iom_put( "zsmltrd_npc"   , smltrd(:,:,jpmld_npc)                         )    ! non penetrative convective adjustment 
            !CALL iom_put( "zsmltrd_atf"   , smltrd(:,:,jpmld_atf)                         )    ! Asselin filter trend
            !CALL iom_put( "zsmltrd_xyad"  , smltrd(:,:,jpmld_xad) + smltrd(:,:,jpmld_yad) )    ! zon.   + mer.   advective trends
            !CALL iom_put( "zsmltrd_lzdf"  , smltrd(:,:,jpmld_ldf) + smltrd(:,:,jpmld_zdf) )    ! horiz. + verti. diffusive trends

         ELSE  ! running average
 
            ! Temperature values
            CALL iom_put( "ztml_sum"       , tml_sum(:,:)                                      )    ! temperature
            CALL iom_put( "ztmltrd2_tot"   , ztmltot2(:,:)                                     )    ! total trends
            CALL iom_put( "ztmltrd2_res"   , ztmlres2(:,:)                                     )    ! residual trends
            CALL iom_put( "ztmltrd2_xad"   , ztmltrd2(:,:,jpmld_xad)                           )    ! zonal. advective trend
            CALL iom_put( "ztmltrd2_yad"   , ztmltrd2(:,:,jpmld_yad)                           )    ! merid. advective trend
            CALL iom_put( "ztmltrd2_zad"   , ztmltrd2(:,:,jpmld_zad)                           )    ! verti. advective trend
            CALL iom_put( "ztmltrd2_ldf"   , ztmltrd2(:,:,jpmld_ldf)                           )    ! horiz. diffusive trend
            CALL iom_put( "ztmltrd2_for"   , ztmltrd2(:,:,jpmld_for)                           )    ! forcing trend
            CALL iom_put( "ztmltrd2_zdf"   , ztmltrd2(:,:,jpmld_zdf)                           )    ! verti. diffusive trend
            CALL iom_put( "ztmltrd2_bbc"   , ztmltrd2(:,:,jpmld_bbc)                           )    ! geothermal flux trend
            CALL iom_put( "ztmltrd2_bbl"   , ztmltrd2(:,:,jpmld_bbl)                           )    ! bottom boundary layer (advective/diffusive)
            CALL iom_put( "ztmltrd2_dmp"   , ztmltrd2(:,:,jpmld_dmp)                           )    ! internal restoring trend
            CALL iom_put( "ztmltrd2_npc"   , ztmltrd2(:,:,jpmld_npc)                           )    ! non penetrative convective adjustment 
            CALL iom_put( "ztmltrd2_atf"   , ztmltrd2(:,:,jpmld_atf)                           )    ! Asselin filter trend
            CALL iom_put( "ztmltrd2_xyad"  , ztmltrd2(:,:,jpmld_xad) + ztmltrd2(:,:,jpmld_yad) )    ! zon.   + mer.   advective trends
            CALL iom_put( "ztmltrd2_lzdf"  , ztmltrd2(:,:,jpmld_ldf) + ztmltrd2(:,:,jpmld_zdf) )    ! horiz. + verti. diffusive trends

            ! Salinity values
            !CALL iom_put( "zsml_sum"       , sml_sum(:,:)                                      )    ! salinity
            !CALL iom_put( "zsmltrd2_tot"   , zsmltot2(:,:)                                     )    ! total trends
            !CALL iom_put( "zsmltrd2_res"   , zsmlres2(:,:)                                     )    ! residual trends
            !CALL iom_put( "zsmltrd2_xad"   , zsmltrd2(:,:,jpmld_xad)                           )    ! zonal. advective trend
            !CALL iom_put( "zsmltrd2_yad"   , zsmltrd2(:,:,jpmld_yad)                           )    ! merid. advective trend
            !CALL iom_put( "zsmltrd2_zad"   , zsmltrd2(:,:,jpmld_zad)                           )    ! verti. advective trend
            !CALL iom_put( "zsmltrd2_ldf"   , zsmltrd2(:,:,jpmld_ldf)                           )    ! horiz. diffusive trend
            !CALL iom_put( "zsmltrd2_for"   , zsmltrd2(:,:,jpmld_for)                           )    ! forcing trend
            !CALL iom_put( "zsmltrd2_zdf"   , zsmltrd2(:,:,jpmld_zdf)                           )    ! verti. diffusive trend
            !CALL iom_put( "zsmltrd2_bbc"   , zsmltrd2(:,:,jpmld_bbc)                           )    ! geothermal flux trend
            !CALL iom_put( "zsmltrd2_bbl"   , zsmltrd2(:,:,jpmld_bbl)                           )    ! bottom boundary layer (advective/diffusive)
            !CALL iom_put( "zsmltrd2_dmp"   , zsmltrd2(:,:,jpmld_dmp)                           )    ! internal restoring trend
            !CALL iom_put( "zsmltrd2_npc"   , zsmltrd2(:,:,jpmld_npc)                           )    ! non penetrative convective adjustment 
            !CALL iom_put( "zsmltrd2_atf"   , zsmltrd2(:,:,jpmld_atf)                           )    ! Asselin filter trend
            !CALL iom_put( "zsmltrd2_xyad"  , zsmltrd2(:,:,jpmld_xad) + zsmltrd2(:,:,jpmld_yad) )    ! zon.   + mer.   advective trends
            !CALL iom_put( "zsmltrd2_lzdf"  , zsmltrd2(:,:,jpmld_ldf) + zsmltrd2(:,:,jpmld_zdf) )    ! horiz. + verti. diffusive trends
...
!-lb      END IF
      ...
      ! IV.3 Code for IOIPSL/NetCDF output
      ! ----------------------------------
...
!-lb
#endif        /* NOT key_iomput && NOT key_dimgout */
!-lb
